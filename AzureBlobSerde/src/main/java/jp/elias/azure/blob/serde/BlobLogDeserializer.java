package jp.elias.azure.blob.serde;
...
public class BlobLogDeserializer extends AbstractDeserializer {

    public static final Log LOG = LogFactory.getLog(BlobLogDeserializer.class
            .getName());
    ...
    public String toString() {
        return "BlobLogDeserializer[]";
    }

    public BlobLogDeserializer() throws SerDeException {
    }

    static Pattern regexpat = Pattern
            .compile("(\\S+) (\\S+) \\[(.*?)\\] (\\S+) (\\S+) (\\S+) (\\S+) (\\S+) (\".+\"|-) (\\S+) (\\S+) (\\S+) (\\S+) (\\S+) (\\S+) (\".+\"|-) (\".*\"|-) (.*)");

    LogStruct deserializeCache = new LogStruct();
    ...
    public void initialize(Configuration job, Properties tbl)
        ...
        cachedObjectInspector = ObjectInspectorFactory
                .getReflectionObjectInspector(LogStruct.class,
    ...
    public Object deserialize(LogStruct c, String row) throws Exception {
        ...
        try {
            ...
            unmatchedRows++;
            if (unmatchedRows > 50) {
                throw new SerDeException("Log Regex did not match:" + row, e);
            }
        }
        return (c);
    }
